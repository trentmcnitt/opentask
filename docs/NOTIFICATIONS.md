...
| Failed sends don't use the window           | A send APNs fails for a reason that could pass later (network, 5xx: `sendApnsWidgetReload` resolves `false`) is rolled back from the pacing, so the next change goes out after the 2s settle instead of a full window later. No automatic retry: nothing is sent until something changes again. A stale token is removed and counts as done.                                                                       |
