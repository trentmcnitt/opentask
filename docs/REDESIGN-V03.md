...
> **AMENDED 2026-09-07 (Trent). Everything below this box that says a recurring _task_ is "not current" unless its rrule produces an occurrence today, or that an explicit snooze "wins for the rest of that day only", is retired.** The rule now, in his words:
>
> > _"I would expect to be nagged about it the next day because it's overdue. It's been overdue ever since I didn't get it done. … it's due at the time it's snoozed to, and if I don't do it when the snoozed time comes up, then it's overdue from there. … there's no intuitiveness to me about how something at midnight magically stops being overdue and is waiting for its next recurrence. How else am I going to make sure that it gets done?"_
>
> **A recurring task carries debt.** Its `due_at` is the truth whether ahead (the next occurrence, or an explicit snooze target) or behind (overdue since then, and staying so — nagged at its priority cadence, through midnight, into the next day and the next — until done). Completing it moves `due_at` to the next scheduled occurrence _after the completion_ (`from_due`: `rrule.after(max(prevDue, completedAt))`), so a Mon/Thu task finished on Tuesday next lands on Thursday. Only a recurring task with **no** `due_at` at all derives today's occurrence from the rrule. **Reminders (§6) still roll forward** — a missed one is not re-shown until its next occurrence; that is what the Reminders surface is for. One-offs are unchanged.
>
> _Why the section below said otherwise:_ it was AI-authored in design review (07-27-26) with no attribution to Trent, at a time when every one of the ~217 recurring items was a _task_, kept current only by his daily bulk sweep. Killing the nagging (the point of this document) would have frozen ~175 never-completed protocol items at "overdue by N days" forever, and rolling _everything_ forward was the only lever before the Reminders surface existed. It contradicted his own recorded rule of 07-22-26 — _"if missed today, does it need making up? no → rhythm (rolls forward), yes → task (STAYS OVERDUE)"_ — for the whole "yes" branch. With reminders live and holding the rhythm population, the reason is gone; when he found the behaviour on 09-07 and asked for a record of ever wanting it, there was none. Implemented in `src/core/recurrence/occurrence.ts` (`effectiveDueAt`); OC-004/005/006/009 and DV-007 flipped to pin the new rule.
