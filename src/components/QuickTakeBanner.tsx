...
export function QuickTakeBanner({ text, loading, onDismiss }: QuickTakeBannerProps) {
  // Auto-dismiss after 12 seconds — only when showing text, not during loading
  useEffect(() => {
    if (!text || loading) return
    const timer = setTimeout(onDismiss, 12000)
    return () => clearTimeout(timer)
  }, [text, loading, onDismiss])
