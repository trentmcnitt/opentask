...
- **Deep links**: `opentask://today`, `opentask://reminders`, `opentask://reminder/<id>`, `opentask://task/<id>` — resolved to web paths in `OpenTaskApp.handleWidgetLink`. A reminder row links to `reminder/<id>` → `/reminders?reminder=<id>` (its own surface, row brought into view and highlighted, no editor); `task/<id>` → `/?task=<id>` (the dashboard with the task's editor open) and is for tasks only.
