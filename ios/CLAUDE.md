...
- **Row counts**: the Reminders and Tasks lists do NOT have a fixed row count. `ViewThatFits` walks down from a per-family ceiling and renders the first card that fits, and each row RESERVES its lines (`WidgetTheme.rowTitleLineHeight`) so that measurement is honest — ViewThatFits compares _ideal_ heights, and a Text's ideal height is one unwrapped line. Without the reservation the tallest candidate always "fits", the card then squeezes, and every title truncates to one line. Titles wrap to two lines in `systemLarge`, one in `systemMedium` (a 4×2 is ~128pt of usable height and holds a header plus two rows, nothing more). **This iOS description is the fixed-budget model only** — on the macOS build (`macos/OpenTaskMacWidgets`, same source files), `systemLarge` rows instead reserve the title's REAL measured line count (`WidgetTheme.measuredLineCount`, real AppKit text measurement) rather than always assuming 2, `lineLimit` is unbounded so nothing ever ellipsizes, and the row ceiling is 10 rather than 6 to match the shorter rows that produces — see the "macOS row-height truthing (2026-09-22)" comment block in `WidgetTheme.swift` for the full diagnosis and why iOS wasn't touched.
